...
bool divisibles(int *prices, int size, int first, int second)
{
    ...
    else if (divisibles(prices, size -1, first + prices[size -1], second))
    {
        return true;
    }
    else if (divisibles(prices, size -1, first, second + prices[size -1]))
    {
        return true;
    }
    else{
...
bool divisible(int *prices, int size)
{
    ...
    {
        int divisible = sum / 2;
        bool result = divisibles(prices, size, divisible, divisible);
        return result;
    }
    else if (sum % 2 != 0)
    {
